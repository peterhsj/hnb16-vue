<template>
  <div>
    <!-- 押匯單據查詢列表 -->
    <div class="d-flex justify-space-between align-center">
      <h1 class="hnb__title">{{ props.title }}</h1>

      <div v-if="totalAmount > 0">
        <span class="me-5">
          <span class="text-teal-darken-2">發票張數總計：</span>
          {{ thousandsFormatting(totalAmount) }} 張
        </span>

        <v-btn
          class="me-5 ms-4"
          color="light-blue-darken-2"
          size="small"
          @click="handleExportExcel"
        >
          <v-icon class="me-2" icon="mdi-file-excel-outline" size="16" />
          下載 Excel 檔
        </v-btn>
      </div>
    </div>

    <v-card class="border-lg pa-4" flat>
      <div v-if="totalAmount > 0" class="mb-4">
        <v-btn
          class="ma-2"
          color="deep-orange-darken-1"
          size="small"
          @click="handleDelete"
        >
          <v-icon :icon="'mdi-trash-can-outline'" size="18" />
          刪除
        </v-btn>

        <v-btn
          color="success"
          size="small"
          @click="handleNegoResubmission"
        >
          <v-icon class="me-2" icon="mdi-receipt-send-outline" size="16" />
          押匯重新提示
        </v-btn>
      </div>
      <!-- 無資料時顯示 -->
      <v-card v-if="!loading && items.length === 0" class="mx-auto my-3" color="grey-lighten-4" flat>
        <v-card-text class="text-center py-12">
          <v-icon color="grey-lighten-1" icon="mdi-file-document-alert-outline" size="64" />
          <p class="text-h6 text-orange-darken-2 mt-4">尚無資料</p>
        </v-card-text>
      </v-card>

      <v-data-table-server
        v-else
        v-model="selectedItems"
        v-model:items-per-page="pageOptions.itemsPerPage"
        class="hnb__table"
        color="light-blue-darken-2"
        density="compact"
        fixed-header
        :headers="headers"
        :height="tableHeight"
        item-value="id"
        :items="items"
        :items-length="totalCount"
        :loading="loading"
        show-current-page
        show-select
        striped="even"
        @update:options="loadItems"
      >
        <template #header.invoicesRequired>
          <div class="text-end text-no-wrap">應檢附 / 已檢附<br>發票張數</div>
        </template>

        <template #item.invoicesRequired="{ item }">
          {{ item?.invoicesRequired || 0 }} / {{ item?.invoicesAttached || 0 }}
        </template>

        <!-- 匯票號碼 -->
        <template #item.draftNo="{ item }">
          <a v-if="item.draftNo" class="text-blue-darken-2" href="#" @click.prevent="handleDraftView(item.draftNo)">
            {{ item.draftNo }}
          </a>

          <span v-else>-</span>
        </template>

        <!-- 買受人名稱 / 統編 -->
        <template #item.buyerName="{ item }">
          <span class="text-caption text-grey-darken-1">{{ item.applicantId || '-' }}</span><br>
          {{ item.buyerName || '-' }}
        </template>

        <!-- 信用狀號碼 -->
        <template #item.lcNo="{ item }">
          <a v-if="item.lcNo" class="text-blue-darken-2" href="#" @click.prevent="handleLcView(item.lcNo)">
            {{ item.lcNo }}
          </a>

          <span v-else>-</span>
        </template>

        <!-- 押匯日期 / 預約押匯日期 -->
        <template #header.issuingDate>
          <div class="text-center text-no-wrap">預約押匯日期<br>押匯日期</div>
        </template>

        <template #item.issuingDate="{ item }">
          {{ item.scheduledIssuingDate || '' }}<br>
          {{ item.issuingDate || '-' }}
        </template>

        <!-- 匯票日期 / 匯票到期日 -->
        <template #header.draftDate>
          <div class="text-center text-no-wrap">匯票日期<br>匯票到期日</div>
        </template>

        <template #item.draftDate="{ item }">
          {{ item.draftDate || '' }}<br>
          {{ item.draftDueDate || '-' }}
        </template>

        <!-- 押匯金額 -->
        <template #item.issuingAmount="{ item }">
          $ {{ thousandsFormatting(item?.issuingAmount || 0) }}
        </template>

        <!-- 操作 -->
        <template #item.action="{ item }">
          <v-btn
            color="light-blue-darken-2"
            size="small"
            @click="handleEdit(item)"
          >
            <v-icon class="me-2" icon="mdi-pencil" size="16" />
            編輯訊息
          </v-btn>
        </template>
      </v-data-table-server>
    </v-card>
    <!-- lc Dialog -->
    <LcDialog
      v-if="lcDialog"
      v-model:lc-dialog="lcDialog"
      :lc-id="lcId"
      @on-close="lcDialogClose"
    />
    <!-- edit Dialog -->
    <EditDialog
      v-if="editDialog"
      v-model:edit-dialog="editDialog"
      :payload="payload"
      @on-close="editDialogClose"
      @on-save="handleSaveSuccess"
    />
    <!-- Prompt Dialog -->
    <PromptDialog
      v-model:message-dialog="messageDialog"
      :is-confirm-btn="isConfirmBtn"
      :message="message"
      :message-status="messageStatus"
      :message-title="messageTitle"
      @on-close="messageClose"
      @prompt-confirm="messageConfirm"
    />
  </div>
</template>

<script setup lang="ts">
  import { storeToRefs } from 'pinia'
  import { onMounted, onUnmounted, ref, watch } from 'vue'
  import { useRouter } from 'vue-router'
  import { api } from '@/api/axios'
  import { useApiErrorHandler } from '@/composables/useApiErrorHandler'
  import { useUserStore } from '@/stores/user'
  import { thousandsFormatting } from '@/utils/format'

  const userStore = useUserStore()
  const { userInfo } = storeToRefs(userStore)
  const router = useRouter()
  const { handleApiError } = useApiErrorHandler()

  interface Props {
    title?: string
    searchType: string
    formData: {
      issuingBankId: string | null
      issuingBranchId: string | null
      draftNo: string | null
      draftStatus: string | null
    }
  }
  const props = defineProps<Props>()

  // 選中的項目
  const selectedItems = ref<TableDataItem[]>([])

  // 防止重複調用的標記
  const isSearching = ref<boolean>(false)

  // LC Dialog
  const lcDialog = ref<boolean>(false)
  // Edit Dialog
  const editDialog = ref<boolean>(false)

  // 監聽 formData 變化，當有查詢條件時重新載入資料
  watch(
    () => props.formData,
    (newValue: Props['formData']) => {
      // 檢查是否有任何查詢條件
      const hasSearchCriteria = Object.keys(newValue || {}).length > 0
      if (hasSearchCriteria) {
        // 設置標記並重置到第一頁
        isSearching.value = true
        pageOptions.value = { ...pageOptionsInit.value }
        fetchNegotiationDocList().finally(() => {
          isSearching.value = false
        })
      }
    },
    { deep: true })

  // 動態計算表格高度
  const tableHeight = ref<number>(600)

  function calculateTableHeight () {
    // 計算可用高度：視窗高度 - 其他元素高度（如標題、padding 等）
    // 可根據實際情況調整減去的數值
    const offset = 615 // 扣除頁面其他元素的高度
    tableHeight.value = window.innerHeight - offset
  }

  // Prompt Message Dialog
  const messageDialog = ref<boolean>(false)
  const messageTitle = ref<string>('')
  const message = ref<string>('')
  const messageStatus = ref<string>('')
  const isConfirmBtn = ref<boolean>(false)
  const processStatus = ref<string>('')

  onMounted(() => {
    calculateTableHeight()
    window.addEventListener('resize', calculateTableHeight)
  })

  onUnmounted(() => {
    window.removeEventListener('resize', calculateTableHeight)
  })

  interface HeaderItem {
    id?: number
    title: string
    key: string
    sortable?: boolean
    nowrap?: boolean
    headerProps?: {
      class: string
    }
    align?: 'start' | 'end' | 'center' | undefined
  }
  const headers: HeaderItem[] = [
    { title: '押匯銀行', key: 'issuingBank', sortable: false, align: 'start', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '匯票號碼', key: 'draftNo', sortable: false, align: 'start', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '買受人名稱 / 統編', key: 'buyerName', sortable: false, align: 'start', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '信用狀號碼', key: 'lcNo', sortable: false, align: 'center', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '押匯日期', key: 'issuingDate', sortable: false, align: 'center', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '匯票日期', key: 'draftDate', sortable: false, align: 'center', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '押匯金額 ( NT$ )', key: 'issuingAmount', sortable: false, align: 'end', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '應檢附 / 已檢附發票張數', key: 'invoicesRequired', sortable: false, align: 'end', nowrap: false, headerProps: { class: 'bg-blue-lighten-4 py-2' } },
    { title: '狀態', key: 'status', sortable: true, align: 'start', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
    { title: '操作', key: 'action', sortable: false, align: 'center', nowrap: true, headerProps: { class: 'bg-blue-lighten-4' } },
  ]

  const loading = ref<boolean>(false)

  interface TableDataItem {
    id: string
    issuingBank: string // 押匯銀行
    draftNo: string // 匯票號碼
    buyerName: string // 買受人名稱
    applicantId: string // 買受人統編
    lcNo: string // 信用狀號碼
    issuingDate: string // 押匯日期
    draftDate: string // 匯票日期
    scheduledIssuingDate: string // 預約押匯日期
    draftDueDate: string // 匯票到期日
    issuingAmount: number // 押匯金額 ( NT$ )
    invoicesRequired: number // 應檢附發票張數
    invoicesAttached: number // 已檢附發票張數
    status: string // 狀態
  }
  const items = ref<TableDataItem[]>([])
  /** 分頁 **/
  interface PageOptions {
    page: number
    itemsPerPage: number
    sortBy: {
      key: string
      order?: 'asc' | 'desc'
    }[]
  }
  const pageOptionsInit = ref<PageOptions>({
    page: 1,
    itemsPerPage: 10,
    sortBy: [{ key: 'id', order: 'asc' }],
  })
  const pageOptions = ref<PageOptions>({ ...pageOptionsInit.value })
  const totalCount = ref<number>(0) // 總筆數
  const totalAmount = ref<number>(0) // 總計發票張數

  function loadItems ({ page, itemsPerPage, sortBy }: PageOptions): void {
    // 如果正在執行搜索，跳過此次調用
    if (isSearching.value) {
      return
    }
    pageOptions.value.page = page
    pageOptions.value.itemsPerPage = itemsPerPage
    pageOptions.value.sortBy = sortBy.map(s => ({ key: s.key, order: s.order }))
    fetchNegotiationDocList()
  }
  // 取得押匯單據列表
  async function fetchNegotiationDocList (): Promise<void> {
    const { page, itemsPerPage, sortBy } = pageOptions.value
    const payload = {
      ...props.formData, // 包含查詢條件
      page,
      itemsPerPage,
      sortBy: sortBy.map(s => s.key),
      sortDesc: sortBy.map(s => s.order === 'desc'),
    }
    loading.value = true
    const apiUrl = '/api/negotiationDoc/list'
    try {
      if (userInfo.value.token) {
        const res = await api.post(apiUrl, payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
        const { status, data } = res
        const { data: resData, total, totalAmount: amount } = data

        console.log('API 回應:', { resData, total, payload })
        // 更新表格數據
        if (status === 200) {
          items.value = resData
          totalCount.value = total
          totalAmount.value = amount // 更新總計發票張數
        }
      } else {
        router.push('/login')
        return
      }
    } catch (error: any) {
      await handleApiError(error, fetchNegotiationDocList, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
        messageDialog,
      })
    } finally {
      loading.value = false
    }
  }

  // 下載 Excel 檔
  async function handleExportExcel (): Promise<void> {
    const payload = {
      ...props.formData,
    }
    try {
      if (userInfo.value.token) {
        const res = await api.post('/api/negotiationDoc/export', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
        const { status, data } = res
        const { message: apiMessage, data: apidata } = data

        if (status === 200) {
          messageTitle.value = '訊息通知'
          message.value = apiMessage || `匯出成功，共 ${apidata.exportCount} 筆資料`
          messageStatus.value = 'success'
          isConfirmBtn.value = false
          messageDialog.value = true

          // 實際應用中，這裡應該觸發檔案下載
          // 例如：window.open(data.fileUrl, '_blank')
          console.log('匯出檔案:', data.fileName, data.fileUrl)
        } else {
          throw new Error(apiMessage || '匯出失敗')
        }
      } else {
        router.push('/login')
        return
      }
    } catch (error: any) {
      await handleApiError(error, handleExportExcel, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
        messageDialog,
      })
    }
  }

  // 刪除選中項目
  async function handleDelete (): Promise<void> {
    if (selectedItems.value.length === 0) {
      messageTitle.value = '訊息通知'
      message.value = `請先選擇要刪除的項目`
      messageStatus.value = 'alert'
      isConfirmBtn.value = false
      messageDialog.value = true
      return
    }

    // 顯示確認對話框
    processStatus.value = 'delete'
    messageTitle.value = '確認刪除'
    message.value = `確定要刪除選中的 ${selectedItems.value.length} 筆資料嗎？`
    messageStatus.value = 'alert'
    isConfirmBtn.value = true
    messageDialog.value = true
  }

  // 實際執行刪除
  async function executeDelete (): Promise<void> {
    const ids = selectedItems.value.map(item => item.id)
    try {
      if (userInfo.value.token) {
        const res = await api.post('/api/negotiationDoc/delete', { ids }, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
        const { status, data } = res
        const { message: apiMessage } = data

        if (status === 200) {
          messageTitle.value = '訊息通知'
          message.value = apiMessage || '刪除成功'
          messageStatus.value = 'success'
          isConfirmBtn.value = false
          messageDialog.value = true

          // 清空選中項目並重新載入列表
          selectedItems.value = []
          await fetchNegotiationDocList()
        }
      } else {
        router.push('/login')
        return
      }
    } catch (error: any) {
      await handleApiError(error, executeDelete, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
        messageDialog,
      })
    }
  }

  // 押匯重新提示
  async function handleNegoResubmission (): Promise<void> {
    if (selectedItems.value.length === 0) {
      messageTitle.value = '訊息通知'
      message.value = `請先選擇要重新提示的項目`
      messageStatus.value = 'alert'
      isConfirmBtn.value = false
      messageDialog.value = true
      return
    }

    // 顯示確認對話框
    processStatus.value = 'resubmission'
    messageTitle.value = '確認重新提示'
    message.value = `確定要重新提示選中的 ${selectedItems.value.length} 筆資料嗎？`
    messageStatus.value = 'alert'
    isConfirmBtn.value = true
    messageDialog.value = true
  }

  // 實際執行重新提示
  async function executeResubmission (): Promise<void> {
    const ids = selectedItems.value.map(item => item.id)
    try {
      if (userInfo.value.token) {
        const res = await api.post('/api/negotiationDoc/resubmission', { ids }, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
        const { status, data } = res
        const { message: apiMessage } = data

        if (status === 200) {
          messageTitle.value = '訊息通知'
          message.value = apiMessage || '重新提示成功'
          messageStatus.value = 'success'
          isConfirmBtn.value = false
          messageDialog.value = true

          // 清空選中項目並重新載入列表
          selectedItems.value = []
          await fetchNegotiationDocList()
        }
      } else {
        router.push('/login')
        return
      }
    } catch (error: any) {
      await handleApiError(error, executeResubmission, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
        messageDialog,
      })
    }
  }

  // 編輯訊息
  interface Payload {
    id: string
    draftNo: string
    draftDueDate: string
  }
  const payload = ref<Payload>({
    id: '',
    draftNo: '',
    draftDueDate: '',
  })
  async function handleEdit (item: TableDataItem): Promise<void> {
    const { id, draftNo, draftDueDate } = item
    payload.value = { id, draftNo, draftDueDate }
    editDialog.value = true
  }

  // 查看匯票
  async function handleDraftView (draftNo: string): Promise<void> {
    console.log('查看匯票:', draftNo)
    // 開啟新視窗顯示匯票資訊
    const url = `#/draftInfo?draftId=${encodeURIComponent(draftNo)}`
    const width = 1000
    const height = 800
    const left = (screen.width - width) / 2
    const top = (screen.height - height) / 2
    window.open(url, '_blank', `width=${width},height=${height},left=${left},top=${top}`)
  }

  // 查看 LC 詳細資料
  const lcId = ref<string>('') // 用於存儲當前查看的 LC ID
  function handleLcView (lcNoParam: string): void {
    console.log('查看 LC:', lcNoParam)
    lcId.value = lcNoParam
    lcDialog.value = true
  }

  // 離開 LC Dialog
  function lcDialogClose (): void {
    lcDialog.value = false
  }

  // 離開 Edit Dialog
  function editDialogClose (): void {
    editDialog.value = false
  }

  // 儲存成功後重新載入列表
  function handleSaveSuccess (): void {
    fetchNegotiationDocList()
    messageTitle.value = '訊息通知'
    message.value = `儲存成功`
    messageStatus.value = 'success'
    isConfirmBtn.value = false
    messageDialog.value = true
  }

  // 確認 message
  function messageConfirm (): void {
    messageDialog.value = false
    // 執行刪除的 API 呼叫
    if (processStatus.value === 'delete') {
      executeDelete()
      processStatus.value = ''
    }
    // 執行重新提示的 API 呼叫
    if (processStatus.value === 'resubmission') {
      executeResubmission()
      processStatus.value = ''
    }
  }

  // 離開 message
  function messageClose (): void {
    messageDialog.value = false
  }
</script>
