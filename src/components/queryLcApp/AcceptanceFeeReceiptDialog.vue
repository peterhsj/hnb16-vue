          ...
          <v-card class="hnb__card--bordered" flat rounded="0">
            <v-container>
              <v-row class="mt-3" no-gutters>
                ...
                <v-col class="my-1" cols="6">
                  <span class="font-weight-bold">列印日期：</span>
...
<script setup lang="ts">
  ...
  interface Props {
    isAcceptanceFeeReceiptDialog?: boolean
    lcNo?: string
  }
  const props = withDefaults(defineProps<Props>(), {
    isAcceptanceFeeReceiptDialog: false,
    lcNo: '',
  })

  const show = ref<boolean>(props.isAcceptanceFeeReceiptDialog)
  watch(
    () => props.isAcceptanceFeeReceiptDialog,
    newVal => {
      show.value = newVal
    },
  )
  watch(
    () => show.value,
    newVal => {
      emit('update:isAcceptanceFeeReceiptDialog', newVal)
    },
  )
  ...
  const emit = defineEmits<{
    'update:isAcceptanceFeeReceiptDialog': [boolean]
    'on-close': []
  }>()
