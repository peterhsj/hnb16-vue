  ...
  <div>
    <v-card class="border-sm pa-4 bg-grey-lighten-4" variant="outlined">
      ...
      >
        ...
        <!-- 查看轉帳收入傳票 -->
        <template #item.incomeTransferVoucher="{ item }">
          <v-btn
            v-if="item.incomeTransferVoucher"
            class="hnb__btn--default"
            density="comfortable"
            icon="mdi-file-document-outline"
            size="small"
            variant="flat"
            @click="handleIncomeTransferVoucherView(item.lcNo)"
          />
        </template>
...
<script setup lang="ts">
  import type { ListItem, QueryFormPayload } from '@/types/queryCancelApp'
  ...
  const tableHeaders: DataTableHeader[] = [
    ...
    { title: '轉帳收入傳票', key: 'incomeTransferVoucher', align: 'center', sortable: false, nowrap: false, minWidth: 90 },
  ]
