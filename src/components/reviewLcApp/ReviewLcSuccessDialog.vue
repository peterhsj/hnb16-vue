...
<script setup lang="ts">
  ...
  const tableItems = ref<any[]>([
    { lcAppNo: '099700017161000861-A-01', applicant: '網際測試股份有限公司A', beneficiary: '網際測試股份有限公司', reviewAction: '核准', executionResult: '審核完成' },
  ])

  interface Props {
    isReviewLcApprove?: boolean
    lcAppNo?: string
  }
  const props = withDefaults(defineProps<Props>(), {
    isReviewLcApprove: false,
    lcAppNo: '',
  })

  const show = ref<boolean>(props.isReviewLcApprove)
  watch(
    () => props.isReviewLcApprove,
    newVal => {
      show.value = newVal
    },
  )
  watch(
    () => show.value,
    newVal => {
      emit('update:isReviewLcApprove', newVal)
    },
  )
  ...
  const emit = defineEmits<{
    'update:isReviewLcApprove': [boolean]
    'on-close': []
  }>()
