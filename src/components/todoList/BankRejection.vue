  ...
  <div>
    ...
    >
      <template #item.info="{ item }">
        <div class="py-2">
          <p class="mb-3">[ {{ item.id }} ] 下列 銀行押匯拒絕通知，請查核。</p>
...
<script setup lang="ts">
  ...
  // 取得銀行押匯拒絕通知列表
  async function fetchBankRejectionList (): Promise<void> {
    ...
    loading.value = true
    const apiUrl = '/api/bankRejection/list'
    try {
      ...
      await handleApiError(error, fetchBankRejectionList, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
  ...
  async function executeDelete (): Promise<void> {
    ...
    try {
      if (userInfo.value.token) {
        const res = await api.post<ApiResponse>('/api/bankRejection/delete', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
  ...
  async function executeSendNotification (): Promise<void> {
    ...
    try {
      if (userInfo.value.token) {
        const res = await api.post<ApiResponse>('/api/bankRejection/sendNotification', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
  ...
  onMounted(async () => {
    ...
    await fetchBankRejectionList().finally(() => {
