  ...
  <div>
    ...
    >
      <template #item.info="{ item }">
        <div class="py-2">
          <p class="mb-3">[ {{ item.id }} ] 下列 銀行押匯放行通知，請查核。</p>
...
<script setup lang="ts">
  ...
  // 取得銀行押匯放行通知列表
  async function fetchBankReleaseList (): Promise<void> {
    ...
    loading.value = true
    const apiUrl = '/api/bankRelease/list'
    try {
      ...
      await handleApiError(error, fetchBankReleaseList, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
  ...
  async function executeDelete (): Promise<void> {
    ...
    try {
      if (userInfo.value.token) {
        const res = await api.post<ApiResponse>('/api/bankRelease/delete', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
  ...
  async function executeSendNotification (): Promise<void> {
    ...
    try {
      if (userInfo.value.token) {
        const res = await api.post<ApiResponse>('/api/bankRelease/sendNotification', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
