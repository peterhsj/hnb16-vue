...
<script setup lang="ts">
  ...
  // 取得押匯文件齊全列表
  async function fetchDocumentCompleteList (): Promise<void> {
    ...
    loading.value = true
    const apiUrl = '/api/documentComplete/list'
    try {
      ...
      await handleApiError(error, fetchDocumentCompleteList, {
        messageTitle,
        message,
        messageStatus,
        isConfirmBtn,
  ...
  async function executeDelete (): Promise<void> {
    ...
    try {
      if (userInfo.value.token) {
        const res = await api.post<ApiResponse>('/api/documentComplete/delete', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
        ...
        if (status === 200) {
          ...
          await fetchDocumentCompleteList()
        }
      } else {
        router.push('/login')
        return
  ...
  async function executeSendNotification (): Promise<void> {
    ...
    try {
      if (userInfo.value.token) {
        const res = await api.post<ApiResponse>('/api/documentComplete/sendNotification', payload, {
          headers: {
            Authorization: `Bearer ${userInfo.value.token}`,
          },
        })
  ...
  onMounted(() => {
    ...
    fetchDocumentCompleteList().finally(() => {
