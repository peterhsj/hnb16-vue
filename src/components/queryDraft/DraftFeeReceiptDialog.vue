...
<script setup lang="ts">
  ...
  interface Props {
    isDraftFeeReceiptDialog?: boolean
    lcNo?: string
  }
  const props = withDefaults(defineProps<Props>(), {
    isDraftFeeReceiptDialog: false,
    lcNo: '',
  })

  const emits = defineEmits<{
    'update:isDraftFeeReceiptDialog': [boolean]
    'on-close': []
  }>()

  const show = computed({
    get: () => props.isDraftFeeReceiptDialog,
    set: (value: boolean) => {
      emits('update:isDraftFeeReceiptDialog', value)
    },
  })
