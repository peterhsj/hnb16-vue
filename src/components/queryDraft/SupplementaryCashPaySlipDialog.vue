...
<script setup lang="ts">
  ...
  interface Props {
    isSupplementaryCashPaySlipDialog?: boolean
    lcNo?: string
  }
  const props = withDefaults(defineProps<Props>(), {
    isSupplementaryCashPaySlipDialog: false,
    lcNo: '',
  })

  const emits = defineEmits<{
    'update:isSupplementaryCashPaySlipDialog': [boolean]
    'on-close': []
  }>()

  const show = computed({
    get: () => props.isSupplementaryCashPaySlipDialog,
    set: (value: boolean) => {
      emits('update:isSupplementaryCashPaySlipDialog', value)
    },
  })
