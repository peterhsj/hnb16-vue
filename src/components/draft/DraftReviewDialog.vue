...
<script setup lang="ts">
  ...
  interface Props {
    isDraftReviewDialog?: boolean
    draftNo?: string
  }
  const props = withDefaults(defineProps<Props>(), {
    isDraftReviewDialog: false,
    draftNo: '',
  })

  const emits = defineEmits<{
    'update:isDraftReviewDialog': [boolean]
    'on-close': []
    'on-show-history-view': [string]
  }>()

  const show = computed({
    get: () => props.isDraftReviewDialog,
    set: (value: boolean) => {
      emits('update:isDraftReviewDialog', value)
    },
  })
