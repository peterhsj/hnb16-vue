<template>
  <div>
    <v-container fluid>
      ...
      <div>
        <h2 class="mx-4 hnb16__title">查詢台塑信用狀統計表</h2>
          ...
          <v-form ref="searchFormRef" @submit.prevent="sendSearchForm">
            <v-row class="ma-0">
              <v-col class="d-flex align-center ga-4" cols="12" lg="4" md="6">
                <div class="text-body-1 font-weight-medium text-no-wrap hnb__form-label">交易年份</div>
                ...
                <v-select
                  v-model="searchForm.transactionMonth"
                  bg-color="white"
                  clearable
                  color="teal-darken-2"
                  density="compact"
      ...
      <div v-if="isShowList" class="mt-4 mx-4">
        ...
        <QueryFpcLcRptList :form-data="propsFormData" />
      </div>
    </v-container>
  </div>
</template>
...
<script setup lang="ts">
  ...
  import { MONTH_ITEMS, type QueryFormPayload, YEAR_ITEMS } from '@/types/queryFpcLcRpt'
  import { createInitialQueryForm } from '@/types/queryFpcLcRpt'

  const breadcrumbs = [
    { title: '首頁', href: '/' },
    { title: '查詢作業' },
    { title: '台塑信用狀統計表', disabled: true },
  ]
  ...
  const searchEnabled = computed(() => {
    return searchForm.transactionYear !== null || searchForm.transactionMonth !== null
  })
