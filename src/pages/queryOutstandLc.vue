  ...
  <div>
    ...
    <v-container fluid>
      ...
      <div v-if="currentView === 'search'">
        <h2 class="mx-4 hnb16__title">查詢未結案信用狀額度與保證金</h2>
          ...
          <v-form ref="searchFormRef" @submit.prevent="sendSearchForm">
            ...
            <v-row class="ma-0">
              <v-col class="d-flex align-center ga-4" cols="12" lg="4" md="6">
                <div class="text-body-1 font-weight-medium text-no-wrap hnb__form-label">申請人名稱</div>
      ...
      <div v-if="isShowList" class="mt-4 mx-4">
        ...
        <QueryOutstandLcList :form-data="propsFormData" />
      </div>
    </v-container>
  </div>
</template>
...
<script setup lang="ts">
  import type { QueryFormPayload } from '@/types/queryOutstandLc'
  import { computed, reactive, ref } from 'vue'
  import { VForm } from 'vuetify/components'
  import { createInitialQueryForm } from '@/types/queryOutstandLc'

  const breadcrumbs = [
    { title: '首頁', href: '/' },
    { title: '查詢作業' },
    { title: '未結案信用狀額度與保證金', disabled: true },
  ]
  ...
  const searchEnabled = computed(() => searchForm.applicantName !== '' || searchForm.applicantTaxId !== '')
