...
<script setup lang="ts">
  ...
  import { createInitialQueryForm, LC_STATUS_ITEMS, type LcData, type QueryFormPayload } from '@/types/expiredLc'

  const breadcrumbs = [
    { title: '首頁', to: '/' },
    { title: '申請作業' },
    { title: 'CDS 過期案件主動註銷', to: '/expiredLc' },
  ]
  ...
  function handleEdit (payload: { appNo: string }): void {
    ...
    lcData.value = { lcNo: payload.appNo }
    currentView.value = ''
    isShowList.value = false
    isEdit.value = true
  }
