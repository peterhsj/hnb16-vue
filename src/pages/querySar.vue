  ...
  <div>
    <v-container fluid>
      ...
      <div v-if="currentView === 'search'">
        <h2 class="mx-4 hnb16__title">查詢主管審核異常</h2>
          ...
          <v-form ref="searchFormRef" @submit.prevent="sendSearchForm">
            ...
            <v-row class="ma-0">
              <v-col class="d-flex align-center ga-4" cols="12" lg="4" md="6">
                <div class="text-body-1 font-weight-medium text-no-wrap hnb__form-label">作業</div>
              ...
              <v-col class="d-flex align-center ga-4" cols="12" lg="4" md="6">
                <div class="text-body-1 font-weight-medium text-no-wrap hnb__form-label">交易序號</div>

                <v-text-field
                  v-model="searchForm.transactionSeqNo"
                  bg-color="white"
                  color="teal-darken-2"
                  density="compact"
                  hide-details="auto"
      ...
      <div v-if="isShowList" class="mt-4 mx-4">
        ...
        <QuerySarList :form-data="propsFormData" />
      </div>
    </v-container>
  </div>
</template>
...
<script setup lang="ts">
  import type { QueryFormPayload } from '@/types/querySar'
