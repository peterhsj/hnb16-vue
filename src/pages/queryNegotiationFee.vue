<template>
  <div>
    <v-container fluid>
      ...
      <div>
        <h2 class="mx-4 hnb16__title">查詢國內信用狀押匯手續費優待明細表</h2>
          ...
          <v-form ref="searchFormRef" @submit.prevent="sendSearchForm">
            <v-row class="ma-0">
              <v-col class="d-flex align-start ga-4" cols="12" lg="5" md="6">
                <div class="text-body-1 font-weight-medium text-no-wrap hnb__form-label pt-2">押匯/承兌日期</div>
      ...
      <div v-if="isShowList" class="mt-4 mx-4">
        ...
        <QueryNegotiationFeeList :form-data="propsFormData" />
      </div>
    </v-container>
  </div>
</template>
...
<script setup lang="ts">
  import type { QueryFormPayload } from '@/types/queryNegotiationFee'
  import { isAfter, isBefore } from 'date-fns'
  import { computed, reactive, ref } from 'vue'
  import { VForm } from 'vuetify/components'
  import { createInitialQueryForm } from '@/types/queryNegotiationFee'

  const breadcrumbs = [
    { title: '首頁', href: '/' },
    { title: '查詢作業' },
    { title: '國內信用狀押匯手續費優待明細表', disabled: true },
  ]
  ...
  const searchEnabled = computed(() => {
    return searchForm.draftDateStart !== '' || searchForm.draftDateEnd !== ''
  })
