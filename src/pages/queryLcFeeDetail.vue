<template>
  <div>
    <v-container fluid>
      ...
      <div>
        <h2 class="mx-4 hnb16__title">查詢信用狀手續費明細表</h2>
      ...
      <div v-if="isShowList" class="mt-4 mx-4">
        <div class="d-flex align-center justify-space-between">
          <h2 class="hnb16__title">
            信用狀手續費明細表清冊 2025/08/01 ~ 2025/10/02
          </h2>
        ...
        <QueryLcFeeDetailList :form-data="propsFormData" />
      </div>
    </v-container>
  </div>
</template>
...
<script setup lang="ts">
  import type { QueryFormPayload } from '@/types/queryLcFeeDetail'
  import { isAfter, isBefore } from 'date-fns'
  import { computed, reactive, ref } from 'vue'
  import { VForm } from 'vuetify/components'
  import { createInitialQueryForm } from '@/types/queryLcFeeDetail'

  const breadcrumbs = [
    { title: '首頁', href: '/' },
    { title: '查詢作業' },
    { title: '信用狀手續費明細表', disabled: true },
  ]
  ...
  const searchEnabled = computed(() => {
    return searchForm.issueDateStart !== '' || searchForm.issueDateEnd !== ''
  })
