  ...
  <div>
    <v-container fluid>
      ...
      <div v-if="isShowApp" class="mt-4 mx-4">
        <h1 class="hnb16__title">審核修改申請書</h1>

        <v-card class="border-sm pa-4 bg-grey-lighten-4" variant="outlined">
          <AmendLcInfo :bene-type="beneType" :data="data" />
          ...
          <v-row class="my-4" justify="center">
            <v-col cols="12" md="6" sm="12">
              <v-card class="border-sm bg-white pa-4 mb-5" elevated="2">
                <div class="d-flex align-center ga-2 mb-2">
                  <span class="text-nowrap text-end" style="width: 120px">信用狀資料：</span>

                  <a class="hnb__text--link" href="#" @click.prevent="handleLcView(data.lcNo)">
                    {{ data.lcNo }}
                  </a>
                </div>
              </v-card>
                ...
                <v-row>
                  ...
                  <v-col class="d-flex flex-column justify-center align-center ga-2" cols="12" md="4">
                    <v-btn
                      ...
                      variant="flat"
                      @click="handleCreditData(data.amendAppNo)"
                    >
                      授信資料
                    </v-btn>
                  </v-col>
                ...
                <v-row>
                  ...
                  <v-col class="d-flex flex-column justify-center align-center ga-2" cols="12" md="4">
                    <v-btn
                      class="hnb__btn--orange mx-1"
                      variant="flat"
                      @click="handlePreviewCreditData(data.amendAppNo)"
                    >
                      授信資料
                    </v-btn>
                  </v-col>
      ...
      <!-- 審核核准 Dialog -->
      <ReviewAmendSuccessDialog
        v-model:is-review-amend-approve="isReviewAmendApprove"
        :amend-app-no="amendAppNo"
        @on-close="closeReviewAmendApproveDialog"
      />
...
<script setup lang="ts">
  ...
  // 審核核准 Dialog
  const isReviewAmendApprove = ref(false)
  ...
  function handleReviewData (): void {
    ...
    if (reviewForm.value.reviewStatus === 'approve') {
      isReviewAmendApprove.value = true
    } else {
      messageTitle.value = '訊息通知'
      message.value = `<span class="font-weight-bold">修改申請書號碼：</span>
  ...
  // 關閉審核核准 Dialog
  function closeReviewAmendApproveDialog (): void {
    isReviewAmendApprove.value = false
    isShowList.value = true
    isShowApp.value = false
  }
