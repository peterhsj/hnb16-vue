import type { LcListResponse, LcTypeOption, ListItem, QueryFormPayload } from '@/types/queryAmendApp'
...
const mockItems: ListItem[] = rawItems.map(r => {
  ...
  const amendAppNo = `AMD2025${month}${day}${String(r.amendAppNoIdx).padStart(3, '0')}`
  const noticeNo = `NTC2025${month}${day}${String(r.noticeNoIdx).padStart(3, '0')}`
...
function filterItems (query: Partial<QueryFormPayload> & { amendAppNo?: string }): ListItem[] {
  let items = [...mockItems]
  ...
  if (query.queryMode === 'criteria') {
    if (query.amendAppNo?.trim()) {
      items = items.filter(item => item.amendAppNo.includes(query.amendAppNo!.trim()))
    }
...
// 查詢修改申請書清冊
Mock.mock('/api/queryAmendApp/list', 'post', (options: any): LcListResponse => {
