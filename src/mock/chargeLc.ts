import type { ListItem, ListResponse } from '@/types/chargeLc'
...
function generateMockItems (count = 10): ListItem[] {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    items.push({
      ...
      appDate: Random.date('yyyy/MM/dd'),
      lcAmount: Random.integer(100_000, 1_000_000),
      lcBalanceAmount: lcBalanceAmount,
      draftNo: `P114${String(Random.integer(100_000, 999_999))}`,
      issuingAmount: issuingAmount,
      beneficiary: Random.pick(APPLICANTS),
...
Mock.mock('/api/chargeLc/list', 'post', (options: any): ListResponse => {
